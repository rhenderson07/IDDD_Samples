...
public class Sprint extends Entity {
	...
	public Sprint(TenantId aTenantId, ProductId aProductId, SprintId aSprintId, String aName, String aGoals,
	...
	public void commit(BacklogItem aBacklogItem) {
		...
		int ordering = backlogItems.size() + 1;

		CommittedBacklogItem committedBacklogItem = new CommittedBacklogItem(tenantId, sprintId,
				aBacklogItem.backlogItemId(), ordering);

		backlogItems.add(committedBacklogItem);
	}
	...
	public void reorderFrom(BacklogItemId anId, int anOrderOfPriority) {
		for (CommittedBacklogItem committedBacklogItem : backlogItems) {
	...
	public void uncommit(BacklogItem aBacklogItem) {
		CommittedBacklogItem cbi = new CommittedBacklogItem(this.tenantId(), this.sprintId(),
	...
	@Override
	public int hashCode() {
		int hashCodeValue = +(11873 * 53) + this.tenantId().hashCode() + this.productId().hashCode()
	...
	public String toString() {
		return "Sprint [tenantId=" + tenantId + ", productId=" + productId + ", sprintId=" + sprintId
				+ ", backlogItems=" + backlogItems + ", begins=" + begins + ", ends=" + ends + ", goals=" + goals
				+ ", name=" + name + ", retrospective=" + retrospective + "]";
	}
