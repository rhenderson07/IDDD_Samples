...
public abstract class EventTrackingTestCase extends TestCase {
	...
	private TestAgilePMRabbitMQExchangeListener agilePmRabbitMQExchangeListener;
	private TestAgilePMSlothMQExchangeListener agilePmSlothMQExchangeListener;
	private TestCollaborationRabbitMQExchangeListener collaborationRabbitMQExchangeListener;
	private TestCollaborationSlothMQExchangeListener collaborationSlothMQExchangeListener;
	private TestIdentityAccessRabbitMQExchangeListener identityAccessRabbitMQExchangeListener;
	private TestIdentityAccessSlothMQExchangeListener identityAccessSlothMQExchangeListener;
	...
	protected void expectedEvent(Class<? extends DomainEvent> aDomainEventType, int aTotal) {
		...
		if (count != aTotal) {
			throw new IllegalStateException("Expected " + aTotal + " " + aDomainEventType.getSimpleName()
					+ " events, but handled " + this.handledEvents.size() + " events: " + this.handledEvents);
		}
	}

	protected void expectedEvents(int anEventCount) {
		if (this.handledEvents.size() != anEventCount) {
			throw new IllegalStateException("Expected " + anEventCount + " events, but handled "
	...
	private abstract class TestExchangeListener
