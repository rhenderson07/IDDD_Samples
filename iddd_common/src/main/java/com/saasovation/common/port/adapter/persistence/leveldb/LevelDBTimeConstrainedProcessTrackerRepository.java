...
public class LevelDBTimeConstrainedProcessTrackerRepository extends AbstractLevelDBRepository
	...
	private List<TimeConstrainedProcessTracker> listAllTrackers() {
		List<TimeConstrainedProcessTracker> allTrackers = new ArrayList<TimeConstrainedProcessTracker>();
		...
		for (Object trackerId : keys) {
			TimeConstrainedProcessTracker tracker = uow.readObject(trackerId.toString().getBytes(),
	...
	private void save(TimeConstrainedProcessTracker aTimeConstrainedProcessTracker, LevelDBUnitOfWork aUoW) {

		LevelDBKey primaryKey = new LevelDBKey(PRIMARY, aTimeConstrainedProcessTracker.tenantId(),
				aTimeConstrainedProcessTracker.processId().id());

		aUoW.write(primaryKey, aTimeConstrainedProcessTracker);

		LevelDBKey allTrackers = new LevelDBKey(primaryKey, ALL_TRACKERS);

		aUoW.updateKeyReference(allTrackers);

		LevelDBKey allTenantTrackers = new LevelDBKey(primaryKey, ALL_TENANT_TRACKERS,
